...
- Ports free locally: **9091** (main-backend), **5173** (web), **5174** (admin-web), **3000** (website), **4173/4174** (vite preview)
...
| -------------- | ---------- | -------- | ---------------------- | ------------------------------ |
| `main-backend` | Express    | 9091     | `pnpm start`           | Public HTTP API (`/api/v1/*`)  |
...
```bash
pnpm -F @repo/main-backend dev        # 9091
...
```bash
curl http://localhost:9091/api/v1/health           # main-backend
```
